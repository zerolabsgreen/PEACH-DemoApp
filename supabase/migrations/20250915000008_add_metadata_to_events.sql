-- Add metadata field to events table (if not already present)
-- This migration ensures the events table has the metadata field for consistency
...
-- Update any existing views that reference events to include metadata
-- Check if the view exists and update it
DO $$
BEGIN
    -- Update ea_certificate_with_documents view to include metadata in events
...
-- Update any other views that might reference events
-- Check for production_sources_with_events view
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'production_sources_with_events' AND table_schema = 'public') THEN
        DROP VIEW IF EXISTS public.production_sources_with_events;
        
        CREATE OR REPLACE VIEW public.production_sources_with_events AS
        SELECT 
            ps.id,
            ps.external_ids,
        ...
        -- Restore grants on view
        GRANT SELECT ON public.production_sources_with_events TO authenticated;
        GRANT SELECT ON public.production_sources_with_events TO anon;
    END IF;
END $$;
